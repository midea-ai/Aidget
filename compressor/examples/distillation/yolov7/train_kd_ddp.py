...
def train(hyp, opt, device, tb_writer=None):
    ...
    t_model = torch.load(opt.t_weights, map_location="cpu")
    ...
    if cuda and rank != -1:
        print("In DDP")
        distiller_instance = DDP(distiller_instance, device_ids=[opt.local_rank], output_device=opt.local_rank,
                                 # nn.MultiheadAttention incompatibility with DDP https://github.com/pytorch/pytorch/issues/26698
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            # Save model
            torch.save(distiller_instance.module.student_net, 'tmp.pt')
            tmp_student_net = torch.load('tmp.pt')
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ckpt = {'epoch': epoch,
                        ...
                        'model': deepcopy(tmp_student_net).half(),
...
if __name__ == '__main__':
    ...
    # distillation
    parser.add_argument('--t_weights', type=str, default="yolo-v7.pt", help='teacher model for distillation')
