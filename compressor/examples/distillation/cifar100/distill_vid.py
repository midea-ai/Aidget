...
def train(modelS, device, train_loader, criterion, optimizer, epoch, distiller):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, kd_loss = distiller.forward(data)
...
if __name__ == '__main__':
    ...
    modelT = teacher
    modelT.load_state_dict(torch.load(os.path.join('/data/team/nl/mslim_model_dir/pretrain'
                                                   '/cifar100/{}'.format(teacher_model_selection),
                                                   "{}_cifar100_epoch_240.pt".format(teacher_model_selection))))
    modelS.load_state_dict(torch.load(os.path.join('/data/team/nl/mslim_model_dir/pretrain'
                                                   '/cifar100/{}'.format(student_model_selection),
                                                   "{}_cifar100_epoch_240.pt".format(student_model_selection))))
